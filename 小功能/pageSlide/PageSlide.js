...
var PageSlide = function(el, swipe, pagenation, timer, options) {
	...
	this.pagenation = pagenation || ''; // 是否需要页码
	this.timer = timer || '3000';  // 轮播时间
	this.resize().init().bindEvents(); // 初始化
};

PageSlide.prototype = {
	constructor: PageSlide,
	init: function(i) {
		var This = this,
			current = i ? This.$el.children[i] : This.$el.firstElementChild;
		...
		// pagenation
		var span = This.pagenation.getElementsByTagName('span');
		for(var x = 0; x < span.length; x++) {
			span[x].index = x;
			span[x].onclick = function() {
				This.pageActive(this.index);
				This.direct(this.index);
			}
		}
		This.autoScroll();
		return This;
	...
	autoScroll: function() {
		...
		var timer = setInterval(function() {
			if(This.current < 4) {
				...
				This.current = 0;
				This.pageActive(This.current);
				This.init();
				clearInterval(timer);
			}
		},This.timer);
	},
	pageActive: function (ind) {
		var span = this.pagenation.getElementsByTagName('span');
		for(var y = 0; y < span.length; y++) {
			span[y].className = '';
			span[ind].className = 'active';
		}
	},
	bindEvents: function() {	// 为页面绑定各种事件的绑定函数
		var This = this;
		window.addEventListener('resize orientationchange', this.resize.bind(this), false);
		'touchstart touchmove touchend touchcancel'.split(' ').forEach(function(_eve) {
			//将四个触控函数（申明在后面）绑定到每个页面
			This.$el.addEventListener(_eve, This[_eve].bind(This), false);
		})
	},
	getCurrent: function() {	//获得当前触控的页面对象
		return this.$el.children[this.current];
	},
	resize: function() {	//初始化时获得设备的宽高
		this.width = this.$el.parentNode.clientWidth;
		this.height = this.$el.parentNode.clientHeight;
		return this;
	},
	random: function() {	//到达任意页面的random()方法
	...
	touchstart: function(e) {	// 四个内建的滑动事件函数,与前面绑定函数相呼应
	...
	setX: function(el, x, unit) {	//动态设定translate3d参数方法
	...
	setCurrent: function(el, i) {	//设置当前触控页面translate3d参数为0的方法
	...
	next: function() {		//调用go()方法前往下一或上一页面
		this.pageActive(this.current + 1);
	...
	reset: function() {	//重置方法,用于初始化以及当前页面的重置
	...
	finish: function(curr, target) {	//滑动完成后删除当前页面.play标记以及为下一页面添加.play标记
		this.flag = null;
		setTimeout(function() {
